...
export default function ThursdayEntry({navigation}) {
  const [notes, setNotes] = useState("");

  // Function to save notes to AsyncStorage
  const saveNotes = async () => {
    try {
      await AsyncStorage.setItem('thursdayNotes', notes);
  ...
  const loadNotes = async () => {
    try {
      const savedNotes = await AsyncStorage.getItem('thursdayNotes');
      if (savedNotes !== null) {
        setNotes(savedNotes);
      }
    } catch (e) {
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
      <SafeAreaView style={styles.container}>
        ...
        <Text style={styles.text}>What do you need to let go of?</Text>
