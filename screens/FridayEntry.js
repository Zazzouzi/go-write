...
export default function FridayEntry({navigation}) {
  const [notes, setNotes] = useState("");

  // Function to save notes to AsyncStorage
  const saveNotes = async () => {
    try {
      await AsyncStorage.setItem('fridayNotes', notes);
  ...
  const loadNotes = async () => {
    try {
      const savedNotes = await AsyncStorage.getItem('fridayNotes');
      if (savedNotes !== null) {
        setNotes(savedNotes);
      }
    } catch (e) {
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
      <SafeAreaView style={styles.container}>
        ...
        <Text style={styles.text}>Did you witness something beautiful today?</Text>
