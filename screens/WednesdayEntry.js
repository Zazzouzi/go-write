...
export default function WednesdayEntry({navigation}) {
  const [notes, setNotes] = useState("");

  // Function to save notes to AsyncStorage
  const saveNotes = async () => {
    try {
      await AsyncStorage.setItem('wednesdayNotes', notes);
  ...
  const loadNotes = async () => {
    try {
      const savedNotes = await AsyncStorage.getItem('wednesdayNotes');
      if (savedNotes !== null) {
        setNotes(savedNotes);
      }
    } catch (e) {
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
      <SafeAreaView style={styles.container}>
        ...
        <Text style={styles.text}>Was there a moment that derailed your day?</Text>
