...
export default function SundayEntry({navigation}) {
  ...
  const saveNotes = async () => {
    try {
      await AsyncStorage.setItem('sundayNotes', notes);
  ...
  const loadNotes = async () => {
    try {
      const savedNotes = await AsyncStorage.getItem('sundayNotes');
      if (savedNotes !== null) {
        setNotes(savedNotes);
      }
    } catch (e) {
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
      <SafeAreaView style={styles.container}>
        ...
        <Text style={styles.text}>Write a letter to someone you hate or admire</Text>
